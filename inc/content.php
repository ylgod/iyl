<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="entry-content">
		...
		<div class="post-thumbnail" style="background-image: url(<?php the_post_thumbnail_url(array(842, 200)); ?>); width: 97.5%; height: 200px;"></div>
	<?php } ?>
		<?php
		if(is_singular()){
			the_content();
			wp_link_pages( array( 'before' => '<nav class="page-link"><i class="hjylfont hjyl-feed1"></i><span>' . __( 'Pages:', 'iyl' ) . '</span>', 'after' => '</nav>' ) );
		}elseif(hjyl_theme_options('is_content')){
			the_content( 
				sprintf( 
